...
// BuildNote 为图文/动图作品生成结果：无需 ASR，配文（desc）即文案。
func BuildNote(work resolver.Work, captured time.Time) (Bundle, error) {
	text := asr.Normalize(work.Description)
	if text == "" {
		text = asr.Normalize(work.Title)
	}
	metadata := Metadata{Source: "douyin", VideoID: work.DouyinWorkID, Author: work.AuthorName, SourceURL: work.CanonicalURL, CapturedAt: captured.UTC(), NormalizedText: text, ProcessingVersion: "note-v1"}
	meta, err := json.MarshalIndent(metadata, "", "  ")
	if err != nil {
		return Bundle{}, err
	}
	markdown := fmt.Sprintf("---\nsource: douyin\nvideo_id: %q\nauthor: %q\nsource_url: %q\ncaptured_at: %q\ncontent_type: note\n---\n\n# %s\n\n## 配文\n\n%s\n", work.DouyinWorkID, work.AuthorName, work.CanonicalURL, captured.UTC().Format(time.RFC3339), safeHeading(work.Title), text)
	return Bundle{Markdown: markdown, Text: text + "\n", Meta: meta}, nil
}
