...
func (t *Transcriber) Run(ctx context.Context, repo Repository, job Job, owner string, workFile ownedfiles.File) error {
	...
	result := map[string]any{"rawText": final.RawText, "normalizedText": final.NormalizedText, "providerRequestId": final.RequestID, "files": resultFiles}; return repo.CompleteTranscription(ctx, job.ID, owner, result, time.Now().UTC())
}
...
func (t *Transcriber) writeResult(ctx context.Context, job Job, name, kind, mime string, body []byte) (ownedfiles.File, error) { relative, temporary, final, err := t.Storage.NewScopedTarget("results", job.UserID, job.ID, filepath.Ext(name)); if err != nil { return ownedfiles.File{}, err }; if err := t.Storage.WriteAtomic(temporary, final, body); err != nil { return ownedfiles.File{}, err }; sum := sha256.Sum256(body); file, err := ownedfiles.NewFile(time.Now().UTC(), job.UserID, job.ID, kind, relative, name, mime, hex.EncodeToString(sum[:]), int64(len(body)), nil); if err != nil { _ = os.Remove(final); return ownedfiles.File{}, err }; if err := t.FileRepo.Create(ctx, file); err != nil { _ = os.Remove(final); return ownedfiles.File{}, err }; return file, nil }
