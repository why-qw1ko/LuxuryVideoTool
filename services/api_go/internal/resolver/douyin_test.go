...
func TestWorkFromRouterDataVideo(t *testing.T) {
	data := map[string]any{"loaderData": map[string]any{"item": map[string]any{
		...
		"video": map[string]any{"duration": float64(1200), "width": float64(1080), "height": float64(1920), "play_addr": map[string]any{"url_list": []any{"https://example.test/video"}}},
		"text_extra": []any{map[string]any{"hashtag_name": "测试"}},
	}}}
	work, ok := workFromTree(data)
	if !ok || work.DouyinWorkID != "123" || work.AuthorName != "作者" || work.VideoURL == "" || len(work.Hashtags) != 1 { t.Fatalf("work = %#v", work) }
}
...
func TestWorkFromRouterDataNotePreservesOrder(t *testing.T) {
	...
	work, ok := workFromTree(data)
	if !ok || work.Type != "note" || len(work.Images) != 2 || work.Images[1].URL != "https://example.test/2" { t.Fatalf("work = %#v", work) }
}
