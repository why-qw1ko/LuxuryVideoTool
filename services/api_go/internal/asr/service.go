...
type ProviderFactory func(context.Context) (Provider, Provider)
type Service struct { primary, fallback Provider; providers ProviderFactory; repo *Repository; budget Budget; now func() time.Time; budgetMu sync.Mutex }
...
func (s *Service) SetProviderFactory(factory ProviderFactory) { s.providers = factory }
...
func (s *Service) TranscribeSegment(ctx context.Context, jobID string, index int, audioPath, audioURL string, duration time.Duration, languages, hotwords []string, progress func(done bool) error) (TranscribeResult, error) {
	...
	primary, fallback := s.primary, s.fallback; if s.providers != nil { primary, fallback = s.providers(ctx) }
	if primary == nil { return TranscribeResult{}, ErrAuth }
	result, err := s.call(ctx, primary, jobID, index, audioPath, audioURL, seconds, estimate, languages, hotwords, progress)
	if err != nil && fallbackAllowed(err) && fallback != nil { if budgetErr := s.checkBudget(ctx, estimate); budgetErr != nil { return TranscribeResult{}, budgetErr }; return s.call(ctx, fallback, jobID, index, audioPath, audioURL, seconds, estimate, languages, hotwords, progress) }
	return result, err
}
